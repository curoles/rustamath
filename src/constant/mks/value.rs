...
impl MksVal {
    ...
    /// assert!(half_speed_of_light.unit == KILOMETERS_PER_HOUR_UNIT);
    /// ```
...
impl std::ops::Mul for MksVal {
    ...
    /// assert!(distance_after_2_weeks.unit == METER_UNIT);
    /// ```
...
impl std::ops::Div for MksVal {
    ...
    /// assert!(traveling_time.unit == SECOND_UNIT);
